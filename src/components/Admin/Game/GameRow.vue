<template>
  <li class="list__item game-list__item" :class="{ 'active': edit }">
    <div class="game-list__box game-list__box--title">
      ...
      <input v-else type="text" v-model="gameTitle" class="game-list__field" />
    </div>
    <div class="game-list__box game-list__box--rate">
      <p v-if="!edit">{{linkGame}}</p>
      <input v-else type="text" v-model="linkGame" class="game-list__field" />
    </div>
    <div class="game-list__box game-list__box--rate" >
      <p v-if="!edit">{{minRateTitle}}</p>
      <input v-else type="text" v-model="minRateTitle" class="game-list__field" />
    </div>
    <div class="game-list__box game-list__box--rate">
      <p v-if="!edit">{{maxRateTitle}}</p>
      <input v-else type="text" v-model="maxRateTitle" class="game-list__field" />
    </div>
    <div class="switch">
      <input type="checkbox" :id="id" class="switch__input" :disabled="!edit" v-model="activeGame"/>
      <label :for="id" class="switch__label" :disabled="!edit"></label>
    </div>
    <div class="game-list__buttons">
      <button class="btn" v-if="!edit" @click.prevent="toggleEdit">
      ...
      <button class="btn" v-else @click.prevent="editGame()">
    ...
    <textarea name="" id="" cols="30" rows="10" v-model="descriptionTitle" v-if="edit" class="form-controll game-list__textarea" />
  </li>
</template>

<script>
export default {
  props: ['title', 'minRate', 'maxRate', 'description', 'id', 'active', 'link'],
  data() {
    return {
      edit: false,
