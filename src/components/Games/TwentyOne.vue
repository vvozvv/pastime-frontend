  ...
  <div class="box">
    ...
    <div class="box-game">
      <div class="box-game__col">
        <div class="twon">
          <div class="twon__box">
            ...
            <ul class="twon__list">
              <li v-show="!started" class="twon__item">
                <p>А ты умный! Но так не работает</p>
                <div class="twon__back"><img src="@/assets/card-texture.png" alt=""></div>
              </li>

              <li v-for="card in userCards" :key="card.id" class="twon__item" :class="{'twon__item--open': card.open}">
          ...
          <div class="twon__box">
            ...
            <ul class="twon__list">
              ...
              <li v-for="card in computerCards" :key="card.id" class="twon__item" :class="{'twon__item--open': card.open}">
                <div class="twon__front">
                  <div class="twon__suit twon__suit--top">
                    <span>{{ card.rank }}</span>
                    {{ card.suit }}
      ...
      <div class="box-game__col box-game__col--description">
        <timer />
        <rate @changedRateParent="changeRate" />

        <button v-show="!started" v-if="isLoggedIn" class="btn box-game__btn start" @click="startGame">Начать</button>
        <router-link v-else :to="{name: 'Auth'}" class="btn box-game__btn start">Начать</router-link>

        <button v-show="started && !endGame" class="btn box-game__btn" @click="getRandomCard(0, true)">Еще</button>
        <button v-show="!endGame && started" class="btn box-game__btn" @click="finishGame">Закончить</button>
      </div>
    </div>
  </div>
</template>
